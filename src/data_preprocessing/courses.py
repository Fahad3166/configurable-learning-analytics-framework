...
def preprocess_courses(
    ...
    """
    Preprocess the OULAD courses dataset according
    ...
    logger.info(
        "Starting preprocessing for courses with %s rows",
        len(dataframe)
    )
    ...
    if missing_columns:
        raise ValueError(
            "Missing expected columns in courses dataset: "
            f"{missing_columns}"
        )
    ...
    logger.info(
        "Data-type validation completed for courses"
    )
    ...
    if duplicate_rule.get("type") == "composite_key":
        ...
        if duplicate_mask.any():
            raise ValueError(
                "Duplicate course-presentation records found for columns "
                f"{key_columns}"
            )
    ...
    logger.info(
        "Completed preprocessing for courses with %s rows",
        len(dataframe)
    )
...
def write_clean_courses(
    dataframe: pd.DataFrame,
    output_path: Path
) -> None:
    """
    Write the cleaned courses dataset to Parquet.
    """
    ...
    logger.info(
        "Clean courses dataset written to %s with %s rows",
        output_path,
        len(dataframe)
    )