...
def preprocess_student_info(
    source_path: Path,
    rules: dict[str, Any]
) -> pd.DataFrame:
    """
    Preprocess the OULAD studentInfo dataset according to
    configured CLAF preprocessing rules.
    """

    dataframe = pd.read_csv(source_path)

    logger.info(
        "Starting preprocessing for studentInfo with %s rows",
        len(dataframe)
    )
    ...
    if missing_columns:
        raise ValueError(
            "Missing expected columns in studentInfo dataset: "
            f"{missing_columns}"
        )

    # ---------------------------------------------------------
    # 2. Apply configured missing-value rules
    # ---------------------------------------------------------

    missing_value_rules = rules.get(
        "missing_value_rules",
        {}
    )
    ...
    # ---------------------------------------------------------
    # 3. Enforce configured data types
    # ---------------------------------------------------------
    ...
    logger.info(
        "Data-type validation completed for studentInfo"
    )

    # ---------------------------------------------------------
    # 4. Validate categorical domains
    # ---------------------------------------------------------
    ...
    if duplicate_rule.get("type") == "composite_key":
        ...
        if not key_columns:
            raise ValueError(
                "Composite-key duplicate rule does not "
                "define any columns"
            )
    ...
    logger.info(
        "Completed preprocessing for studentInfo with %s rows",
        len(dataframe)
    )
...
def write_clean_student_info(
    dataframe: pd.DataFrame,
    output_path: Path
) -> None:
    """
    Write the cleaned studentInfo dataset to Parquet.
    """
    ...
    logger.info(
        "Clean studentInfo dataset written to %s with %s rows",
        output_path,
        len(dataframe)
    )